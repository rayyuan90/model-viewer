...
const waitForEnvmap = (model, mapName) => waitForEvent(model, 'envmap-change', e => e.value ? e.value.name === mapName : false);
const waitForLoadAndEnvmap = (scene, element, mapName) => Promise.all([waitForEvent(element, 'load'), waitForEnvmap(scene.model, mapName)]);
